...
## Protocols for Model Weight Swap


class SwapWeightsRequest(BaseModel):
    """Request to swap model weights from a different checkpoint."""

    new_model_path: str = Field(
        ...,
        description="Path to the new model (local path or HuggingFace model ID)",
    )
    new_revision: str | None = Field(
        default=None,
        description="Optional revision for HuggingFace models",
    )
    swap_tokenizer: bool = Field(
        default=False,
        description="Whether to also swap the tokenizer (not yet implemented)",
    )


class SwapWeightsResponse(BaseModel):
    """Response for a model weight swap request."""

    job_id: str = Field(..., description="Unique identifier for the swap job")
    status: str = Field(..., description="Current status of the swap job")
    new_model_path: str = Field(..., description="Path to the new model")
    new_revision: str | None = Field(default=None, description="Model revision")
...
class SwapJobStatusResponse(BaseModel):
    """Response for checking swap job status."""
