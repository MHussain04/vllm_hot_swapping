...
class Executor(ABC):
    ...
    def swap_weights(
        ...
        """Swap model weights from a different checkpoint.

        This loads weights from a new model checkpoint in-place without
        reinitializing CUDA graphs, KV cache, or other GPU resources.
        The new model must have the same architecture as the currently
        loaded model.
        ...
        """
        time_before_swap = time.perf_counter()
        self.collective_rpc(
            "swap_weights",
            kwargs=dict(new_model_path=new_model_path, new_revision=new_revision),
        )
        time_after_swap = time.perf_counter()
        logger.info(
            "Weight swap completed in %.2f seconds.", time_after_swap - time_before_swap
        )
