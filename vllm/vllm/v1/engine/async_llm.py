...
class AsyncLLM(EngineClient):
    ...
    async def swap_weights(
        ...
        """
        # Clear prefix cache before swapping weights since cached KV values
        # were computed with old weights (only if prefix caching is enabled)
        if self.vllm_config.cache_config.enable_prefix_caching:
            await self.reset_prefix_cache()
        await self.engine_core.swap_weights_async(new_model_path, new_revision)
