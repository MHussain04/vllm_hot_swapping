...
from vllm.v1.worker.model_weight_cache import model_weight_cache
...
class GPUModelRunner(
    ...
    def __init__(
        ...
        # Track currently loaded model for cache management
    ...
    def execute_model(
        ...
        # Update cache timestamp for LRU eviction
        model_weight_cache.update_inference_time(self.current_model_path)
    ...
    def _validate_architecture_for_swap(
        self,
        new_model_path: str,
        new_revision: str | None = None,
    ) -> None:
        """Validate that the new model has compatible architecture for swap.

        This checks that critical architecture attributes match between the
        current model and the new model to ensure weights can be loaded
        in-place without shape mismatches.
        ...
        Raises:
            ValueError: If architecture doesn't match or config can't be loaded
        """
        from transformers import AutoConfig

        # Load the new model's config
        try:
            new_hf_config = AutoConfig.from_pretrained(
                new_model_path,
            ...
            raise ValueError(
                f"Failed to load config from {new_model_path}: {e}"
            ) from e

        current_hf_config = self.model_config.hf_config

        # Check architecture type
        current_arch = getattr(current_hf_config, "architectures", [])
        new_arch = getattr(new_hf_config, "architectures", [])
        if current_arch != new_arch:
            raise ValueError(
                f"Architecture mismatch: current model uses {current_arch}, "
                f"new model uses {new_arch}. Cannot swap weights."
            )

        # Check critical shape-determining attributes
        attrs_to_check = [
            "hidden_size",
            "num_hidden_layers",
            "num_attention_heads",
        ...
        for attr in attrs_to_check:
            current_val = getattr(current_hf_config, attr, None)
            new_val = getattr(new_hf_config, attr, None)
            if (
                current_val != new_val
                ...
                raise ValueError(
                    f"Architecture mismatch for {attr}: current={current_val}, "
                    f"new={new_val}. Cannot swap weights."
                )

        logger.info("Architecture validation passed for %s", new_model_path)
    ...
    def swap_weights(
        ...
        """Swap model weights from a different checkpoint in-place with caching.

        This loads weights from a new model checkpoint without reinitializing
        CUDA graphs, KV cache, or other GPU resources. The new model must have
        the same architecture as the currently loaded model.

        Weights are cached in CPU RAM for fast reloading. On first swap, weights
        are loaded from disk (~770ms) and cached. On subsequent swaps to the same
        model, weights are loaded from cache (~100ms).

        Args:
            new_model_path: Path to the new model (local path or HuggingFace ID)
            new_revision: Optional revision for HuggingFace models

        Raises:
            AssertionError: If model is not yet loaded
            ValueError: If architecture doesn't match or weights can't be loaded
        """
        import dataclasses

        assert getattr(self, "model", None) is not None, (
            "Cannot swap weights before model is loaded."
        )

        # Check if weights are cached
        if model_weight_cache.has_model(new_model_path):
            # Cache HIT - load from cache
            logger.info("Loading weights from CACHE for %s", new_model_path)
            swap_start = time.time()
            weights_iterator = model_weight_cache.get_weights_iterator(new_model_path)
            self.get_model().load_weights(weights_iterator)
            swap_time = time.time() - swap_start
            logger.info(
                "Weight swap completed from CACHE in %.3f seconds for %s",
                swap_time, new_model_path
            )
        else:
            # Cache MISS - load from disk and cache
            logger.info("Validating Architecture")
            self._validate_architecture_for_swap(new_model_path, new_revision)

            logger.info("Loading weights from DISK for %s (not cached)", new_model_path)
            swap_start = time.time()

            # Create a temporary model_config with the new model path
            ...
            # Get weights iterator from disk
            model_loader = get_model_loader(self.load_config)
            disk_iterator = model_loader.get_all_weights(
                new_model_config, self.get_model()
            )

            # Wrap iterator to cache as we load
            weights_iterator = model_weight_cache.cache_as_iterate(
                new_model_path, disk_iterator
            )

            # Load weights to GPU (consumes iterator, triggering caching)
            self.get_model().load_weights(weights_iterator)

            swap_time = time.time() - swap_start
            logger.info(
                "Weight swap completed from DISK in %.3f seconds for %s",
                swap_time, new_model_path
            )

        # Track currently loaded model for cache timestamp updates
